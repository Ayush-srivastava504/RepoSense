// lib/tracker.ts "My Applications" tracker — a lightweight, fully client-side Kanban store
// for jobs the user has saved or applied to. Deliberately kept in localStorage (no
// backend/auth dependency) so it works instantly for guests too, same philosophy as the
// existing guest-JWT flow: never make a visitor sign up before they've gotten value out of
...
export function getTrackedJobs(): TrackedJob[] {
    return Object.values(readAll()).sort((a, b) => new Date(b.savedAt).getTime() - new Date(a.savedAt).getTime());
}
