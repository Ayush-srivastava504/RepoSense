// Module: app/europe-jobs/page.tsx
// Defines component(s)/export(s): JOBS_PER_PAGE, Pagination, EuropeJobsPage
//
//
...
function Pagination({ currentPage, totalPages, search, }: {
    ...
    const getPageUrl = (page: number) => {
        ...
        return `/europe-jobs${params.toString() ? `?${params.toString()}` : ''}`;
    };
    if (totalPages <= 1) {
        return null;
    }
...
export default async function EuropeJobsPage({ searchParams, }: {
    searchParams: {
        search?: string;
        page?: string;
    };
        ...
        <form method="GET" action="/europe-jobs" className="mt-6 sm:mt-8">
          <div className="flex flex-col gap-2 sm:gap-3 sm:flex-row">
            ...
            <div className="flex gap-2 sm:gap-3">
              ...
              {search && (<Link href="/europe-jobs" className="btn flex-1 sm:flex-none px-4 sm:px-6 py-2.5 sm:py-3 text-sm touch-manipulation">
                  Clear
                </Link>)}
            </div>
          </div>
