...
function renderBody(body: string) {
    ...
    return blocks.map((block, i) => {
        if (block.startsWith('## ')) {
            return (<h2 key={i} className="mt-8 text-xl font-medium">
          {block.replace('## ', '')}
        </h2>);
        }
        if (block.startsWith('- ')) {
            const items = block.split('\n').map((l) => l.replace(/^- /, ''));
            return (<ul key={i} className="mt-3 list-disc space-y-1 pl-5">
          {items.map((item, j) => (<li key={j}>{item}</li>))}
        </ul>);
        }
        return (<p key={i} className="mt-4" style={{ color: 'var(--ink-soft)' }}>
        {block}
      </p>);
    });
}
export default function BlogPostPage({ params }: Props) {
    ...
    return (<main className="w-full">
      <Script id={`blog-post-breadcrumb-${post.slug}`} type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(crumbs) }}/>
      <Script id={`blog-post-article-${post.slug}`} type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(articleSchema) }}/>
      {faqSchema && (<Script id={`blog-post-faq-${post.slug}`} type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(faqSchema) }}/>)}
      ...
      <article className="mx-auto w-full max-w-3xl px-3 py-10 sm:px-4 sm:py-14">
        ...
        {post.faq && (<div className="mt-10">
            <h2 className="text-xl font-medium">FAQs</h2>
            {post.faq.map((f, i) => (<div key={i} className="mt-4">
