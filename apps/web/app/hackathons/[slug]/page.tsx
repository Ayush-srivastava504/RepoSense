// Module: app/hackathons/[slug]/page.tsx
// Defines component(s)/export(s): InfoRow, HackathonDetailPage
...
export async function generateMetadata({ params, }: {
    ...
    return {
        ...
        description: hackathon.description?.substring(0, 155) ||
...
function InfoRow({ label, value }: {
    ...
    if (!value && value !== 0)
        return null;
    return (<div className="flex items-center justify-between border-b py-3 text-sm" style={{ borderColor: 'var(--line)' }}>
...
export default async function HackathonDetailPage({ params, }: {
    ...
    if (!hackathon)
        notFound();
    const teamSize = hackathon.team_size_min && hackathon.team_size_max
    ...
    return (<main className="mx-auto max-w-3xl px-4 py-12">
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        {hackathon.participation_mode && (<span className="chip chip-muted text-[11px] uppercase">{hackathon.participation_mode}</span>)}
        {hackathon.is_global ? (<span className="chip chip-muted text-[11px]">Global</span>) : hackathon.country ? (<span className="chip chip-muted text-[11px]">{hackathon.country}</span>) : null}
      ...
      <section className="panel mt-8 p-5">
        ...
        <InfoRow label="Starts" value={hackathon.start_date ? new Date(hackathon.start_date).toLocaleDateString() : undefined}/>
        <InfoRow label="Ends" value={hackathon.end_date ? new Date(hackathon.end_date).toLocaleDateString() : undefined}/>
        <InfoRow label="Prize pool" value={hackathon.prize_pool_text}/>
        <InfoRow label="Team size" value={teamSize}/>
      </section>

      {hackathon.description && (<section className="mt-8">
      ...
      {hackathon.eligibility && (<section className="mt-8">
          <h2 className="mb-2 text-sm font-medium uppercase tracking-wide" style={{ color: 'var(--ink-soft)' }}>
            Who can participate?
          </h2>
          <p className="text-sm leading-7" style={{ color: 'var(--ink-soft)' }}>
      ...
      {hackathon.themes && hackathon.themes.length > 0 && (<section className="mt-8">
          <h2 className="mb-2 text-sm font-medium uppercase tracking-wide" style={{ color: 'var(--ink-soft)' }}>
            Themes
          </h2>
          <div className="flex flex-wrap gap-2">
            {hackathon.themes.map((theme) => (<span key={theme} className="chip chip-muted text-[11px]">
                {theme}
              </span>))}
          </div>
        </section>)}

      {hackathon.submission_requirements && hackathon.submission_requirements.length > 0 && (<section className="mt-8">
          <h2 className="mb-2 text-sm font-medium uppercase tracking-wide" style={{ color: 'var(--ink-soft)' }}>
            Submission requirements
          </h2>
          <ul className="list-disc pl-5 text-sm leading-7" style={{ color: 'var(--ink-soft)' }}>
            {hackathon.submission_requirements.map((req) => (<li key={req}>{req}</li>))}
          </ul>
        </section>)}

      {hackathon.apply_url && (<HackathonApplyButton url={hackathon.apply_url} source={hackathon.source} mode={hackathon.participation_mode}/>)}
