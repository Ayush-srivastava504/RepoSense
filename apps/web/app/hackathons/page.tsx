// Module: app/hackathons/page.tsx
// Defines component(s)/export(s): FILTERS, HackathonsPage
//
//
...
export default async function HackathonsPage({ searchParams, }: {
    ...
    return (<main className="mx-auto max-w-6xl px-4 py-12">
        ...
        <div className="mt-5 flex flex-wrap gap-2">
          {FILTERS.map((filter) => {
            ...
            const href = qs.toString()
                ...
                : '/hackathons';
            const active = (filter.mode ?? '') === (params.mode ?? '') &&
                (filter.country ?? '') === (params.country ?? '') &&
                (filter.global ?? '') === (params.global ?? '');
            return (<Link key={filter.label} href={href} className={active
      ...
      {endingSoon.length > 0 && (<section className="mb-10">
          <h2 className="mb-3 text-sm font-medium uppercase tracking-wide" style={{ color: 'var(--ink-soft)' }}>
            Ending soon
          </h2>

          <div className="flex flex-col gap-2">
            {endingSoon.map((hackathon) => (<Link key={hackathon.id} href={`/hackathons/${hackathon.slug}`} className="panel flex items-center justify-between px-4 py-3 text-sm transition hover:-translate-y-0.5">
      ...
      <section>
        ...
        {hackathons.length === 0 ? (<p className="text-sm" style={{ color: 'var(--ink-soft)' }}>
            No active hackathons match this filter right now — check back
            soon, the crawler refreshes this list daily.
          </p>) : (<div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
