...
// Defines function(s): buildJobPostingSchema, parseFirstNumber
//
    ...
    if (compensationValue) {
        schema.baseSalary = {
            ...
            value: {
                ...
                unitText: /year|annum|lpa/i.test(compensationText || '')
                    ? 'YEAR'
                    : 'MONTH',
            },
        };
    ...
    return (<main className="mx-auto w-full max-w-3xl px-4 py-8 sm:py-10">
      ...
      {job.enriched_overview && (<div className="mt-6">
          ...
          {job.enriched_keywords && job.enriched_keywords.length > 0 && (<div className="mt-3 flex flex-wrap gap-1.5">
              {job.enriched_keywords.map((kw) => (<span key={kw} className="chip chip-muted text-[0.65rem]">
                  {kw}
                </span>))}
            </div>)}
        </div>)}
