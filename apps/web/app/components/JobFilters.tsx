...
export type LocationFilter = 'all' | 'remote' | 'japan';
...
/**
 * Server-rendered filter pills for location (All / Remote / Japan) and
 * role group (Software Engineer / Sales / Finance / Other). Filters are
    ...
    <div className="mt-4 flex flex-col gap-3">
      ...
      <div className="flex flex-wrap items-center gap-1.5 sm:gap-2">
        ...
        {GROUP_OPTIONS.map((opt) => (
          <Link
            key={opt.value}
            href={buildHref(location, opt.value)}
            className={`chip text-[0.7rem] sm:text-xs touch-manipulation ${
...
export function parseLocationFilter(value: string | undefined): LocationFilter {
  return value === 'remote' || value === 'japan' ? value : 'all';
}
