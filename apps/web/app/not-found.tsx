...
// A missing page has no canonical URL of its own, so there's nothing to
// robots-index here — this keeps Google from wasting crawl budget on 404s
// and never lets a stray 404 rank for anything.
  ...
  return (
    <main className="mx-auto max-w-2xl px-4 py-20 text-center">
      ...
      <div className="mt-8 flex flex-wrap justify-center gap-2">
        {links.map((l) => (
          <Link key={l.href} href={l.href} className="chip chip-muted text-sm">
            {l.label}
          </Link>
        ))}
      </div>
