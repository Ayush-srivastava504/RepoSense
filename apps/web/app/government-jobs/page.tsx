// Module: app/government-jobs/page.tsx
// Defines component(s)/export(s): JOBS_PER_PAGE, Pagination, GovernmentJobsPage
//
//
...
function Pagination({ currentPage, totalPages, search, }: {
    ...
    const getPageUrl = (page: number) => {
        ...
        return `/government-jobs${params.toString()
            ? `?${params.toString()}`
            : ''}`;
    };
    if (totalPages <= 1) {
        ...
        <form method="GET" action="/government-jobs" className="mt-6 sm:mt-8">
          <div className="flex flex-col gap-2 sm:gap-3 sm:flex-row">
            ...
            <div className="flex gap-2 sm:gap-3">
              ...
              {search && (<Link href="/government-jobs" className="btn flex-1 sm:flex-none px-4 sm:px-6 py-2.5 sm:py-3 text-sm touch-manipulation">
                  Clear
                </Link>)}
            </div>
          </div>
        ...
        {jobs.length > 0 ? (<>
            <div className="mt-8 sm:mt-10 grid gap-4 sm:gap-6 grid-cols-1 sm:grid-cols-2 xl:grid-cols-3">
              {jobs.map((job, index) => (<div key={job.id} className="contents">
                  <JobCard job={job} basePath="/government-jobs"/>
