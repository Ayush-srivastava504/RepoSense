// Module: app/(auth)/github/page.tsx
// Defines component(s)/export(s): SEVERITY_STYLES, README_STATUS_STEPS, ReadmeGenerateButton, GitHubContent, GitHubPage
// Defines function(s): calcProgress
// Defines type(s): Severity, Issue, QualityMetrics, ReviewResult, FixResult
...
function ReadmeGenerateButton({ onGenerate, disabled, }: {
    ...
    return (<div className="flex flex-col gap-2" style={{ minWidth: 0 }}>
      <button onClick={handleClick} disabled={phase === 'running' || disabled} className={`btn btn-ghost${phase === 'running' ? ' btn-loading' : ''}`} aria-busy={phase === 'running'}>
        {phase === 'running' ? '\u00A0' : phase === 'done' ? '✓ README ready' : 'Generate README'}
      </button>

      {phase === 'running' && (<div className="gen-status" role="status" aria-live="polite">
      ...
      {phase === 'idle' && !disabled && (<p className="text-xs" style={{ color: 'var(--muted)' }}>
          Analyses the full repo — typically 2–3 min.
        </p>)}

      {phase === 'error' && (<p className="chip chip-rust" style={{ fontSize: '0.8125rem', borderRadius: 'var(--radius-sm)', padding: '0.4rem 0.75rem' }} role="alert">
          {errorMsg}
        </p>)}
    </div>);
}
function GitHubContent() {
    ...
    const scoreColor = (score: number) => score >= 90 ? 'var(--green)'
        : score >= 75 ? '#ca8a04'
            : score >= 50 ? '#ea580c'
                : 'var(--rust)';
    return (<>
      ...
      {connectError && (<p className="chip chip-rust mt-4 !inline-block !justify-start" role="alert">
          {connectError}
        </p>)}

      {githubConnecting ? (<div className="mt-12 flex flex-col items-center gap-4">
        ...
        </div>) : !connected ? (<div className="panel mt-8 max-w-sm p-8">
        ...
        </div>) : loadingRepos ? (<p className="mt-8 eyebrow">loading repositories…</p>) : (<div className="mt-8 space-y-6">
          
          <select className="field" style={{ maxWidth: '28rem' }} onChange={(e) => {
          ...
          {selectedRepo && (<div className="grid gap-4 md:grid-cols-2">
              ...
              <div className="panel overflow-hidden">
                ...
                <div className="h-72 overflow-auto p-2 sm:h-80">
                  {currentPath && (<button onClick={() => setCurrentPath(currentPath.split('/').slice(0, -1).join('/'))} className="w-full rounded-[var(--radius-sm)] p-2 text-left text-sm" style={{ color: 'var(--ink-soft)' }} onMouseOver={(e) => (e.currentTarget.style.background = 'var(--paper-dim)')} onMouseOut={(e) => (e.currentTarget.style.background = '')}>
                      ← back
                    </button>)}
                  {files.map((file) => (<button key={file.path} onClick={() => openFile(file)} className="block w-full rounded-[var(--radius-sm)] p-2 text-left text-sm" style={{ color: 'var(--ink)' }} onMouseOver={(e) => (e.currentTarget.style.background = 'var(--paper-dim)')} onMouseOut={(e) => (e.currentTarget.style.background = '')}>
              ...
              <div className="panel-dark overflow-hidden">
                ...
                <pre className="h-72 overflow-auto p-4 text-[0.8125rem] leading-relaxed sm:h-80" style={{ fontFamily: 'var(--font-mono)', color: '#d7d6cf' }}>
          ...
          {selectedRepo && (<div className="flex flex-wrap items-start gap-3">
              <button onClick={runReview} disabled={!code || reviewing} className={`btn btn-primary${reviewing ? ' btn-loading' : ''}`} aria-busy={reviewing}>
                {reviewing ? '\u00A0' : 'Review this file'}
              </button>

              <button onClick={runFix} disabled={!review || fixing} className={`btn btn-secondary${fixing ? ' btn-loading' : ''}`} aria-busy={fixing}>
                {fixing ? '\u00A0' : 'Auto-fix issues'}
              </button>

              <ReadmeGenerateButton onGenerate={generateReadme} disabled={!selectedRepo}/>
            </div>)}

          
          {fixResult && (<div className="panel flex flex-wrap items-center gap-4 p-4">
              <span className={fixResult.success ? 'chip chip-green' : 'chip chip-rust'}>
                {fixResult.success
                    ? `${fixResult.fix_count} fix${fixResult.fix_count !== 1 ? 'es' : ''} applied`
                    : 'Nothing fixed'}
              </span>
              {fixResult.skipped.length > 0 && (<span className="text-sm" style={{ color: 'var(--ink-soft)' }}>
                  {fixResult.skipped.length} skipped
                </span>)}
              {fixResult.success && (<span className="text-sm" style={{ color: 'var(--ink-soft)' }}>
                  Preview updated — re-run Review to check remaining issues.
                </span>)}
            </div>)}

          
          {review && (<div className="panel p-5">
              <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-2xl font-bold tabular-nums" style={{ color: scoreColor(review.quality_metrics.quality_score) }}>
              ...
              <div className="mb-4 flex flex-wrap gap-2">
                {Object.entries(review.quality_metrics.severity_breakdown).map(([sev, count]) => (<span key={sev} className={SEVERITY_STYLES[sev as Severity] ?? 'chip chip-muted'}>
                    {count} {sev}
                  </span>))}
              ...
              <div className="flex flex-col gap-3">
                {review.issues.map((issue, i) => (<div key={i} className="rounded-[var(--radius-sm)] p-3" style={{ background: 'var(--paper-dim)' }}>
                    <div className="mb-1 flex flex-wrap items-center gap-2">
                      ...
                      {issue.cwe && (<span className="text-xs" style={{ color: 'var(--ink-soft)' }}>{issue.cwe}</span>)}
                    ...
                    {issue.snippet && (<pre className="mt-2 overflow-auto rounded p-2 text-xs" style={{ background: 'var(--paper)', fontFamily: 'var(--font-mono)', color: 'var(--ink-soft)' }}>
                        {issue.snippet}
                      </pre>)}
                  </div>))}
                {review.issues.length === 0 && (<p className="text-sm" style={{ color: 'var(--green)' }}>No issues found. Code looks clean ✓</p>)}
              </div>
            </div>)}

          
          {showReadme && generatedReadme && (<div className="panel p-5" ref={readmeRef} id="readme-output">
              <div className="mb-4 flex items-center justify-between gap-3 flex-wrap">
                <p className="eyebrow eyebrow-accent">// generated readme</p>
                <div className="flex items-center gap-2">
                  <button onClick={() => navigator.clipboard.writeText(generatedReadme)} className="btn btn-secondary text-xs !py-1 !px-3">
                    Copy
                  </button>
                  <button onClick={() => setShowReadme(false)} className="btn btn-ghost text-xs !py-1 !px-3">
                    Close
                  </button>
                </div>
              </div>
              <pre className="max-h-[30rem] overflow-auto rounded p-4 text-[0.875rem] leading-relaxed" style={{
