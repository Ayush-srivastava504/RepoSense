// Module: app/(auth)/linkedin/page.tsx
// Defines component(s)/export(s): Field, LinkedInContent, LinkedInPage
// Defines function(s): emptyExperience, emptyEducation, scoreColor
// Defines type(s): ExperienceEntry, EducationEntry, Status, RuleResult, AnalysisResult, HistoryEntry
...
function scoreColor(score: number) {
    ...
    if (score >= 65)
        return 'var(--indigo)';
    if (score >= 40)
        return 'var(--score-orange, #c2410c)';
    return 'var(--rust)';
}
...
function LinkedInContent() {
    ...
    const updateEdu = (i: number, field: keyof EducationEntry, value: string) => setEducation((p) => p.map((e, idx) => idx === i ? { ...e, [field]: value } : e));
    ...
    return (<>
      ...
      {!loadingStatus && status && (<div className="panel mt-6 flex flex-col gap-3 p-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
          ...
          {!status.is_pro && status.free_remaining === 0 && status.ad_credits === 0 && (<div className="flex flex-wrap gap-2">
      ...
      <div className="mt-8 grid gap-8 lg:grid-cols-[1fr_320px]">
        <div className="space-y-8">
          <div className="panel p-6 space-y-4">
            ...
            <Field label="Headline">
              <input className="field" value={headline} onChange={(e) => setHeadline(e.target.value)} placeholder="Backend Engineer | Python & Go | Building fintech APIs at scale"/>
            </Field>
            <Field label="About section">
              <textarea className="field min-h-[120px]" value={about} onChange={(e) => setAbout(e.target.value)} placeholder="What you do, the impact you've had, and what you're looking for next…"/>
            </Field>
          ...
          <div className="panel p-6 space-y-4">
            ...
            {experience.map((e, i) => (<div key={i} className="space-y-3 rounded-[var(--radius-md)] border p-4" style={{ borderColor: 'var(--line)' }}>
                ...
                <div className="space-y-2">
                  {e.bullets.map((b, bi) => (<input key={bi} className="field" placeholder="Achievement bullet" value={b} onChange={(ev) => updateExpBullet(i, bi, ev.target.value)}/>))}
                  <button onClick={() => addExpBullet(i)} className="text-xs" style={{ color: 'var(--indigo)' }}>
                    + Add bullet
                  </button>
                </div>
                {experience.length > 1 && (<button onClick={() => removeExp(i)} className="text-xs" style={{ color: 'var(--rust)' }}>Remove</button>)}
              </div>))}
          </div>
          ...
          <div className="panel p-6 space-y-4">
            ...
            {education.map((e, i) => (<div key={i} className="grid gap-3 sm:grid-cols-2">
                ...
                <div className="flex gap-2">
                  ...
                  {education.length > 1 && (<button onClick={() => removeEdu(i)} className="text-xs flex-shrink-0" style={{ color: 'var(--rust)' }}>Remove</button>)}
                </div>
              </div>))}
          </div>
          ...
          {error && (<div className="panel p-4 text-sm" style={{ borderColor: 'var(--rust)', color: 'var(--rust)' }}>
              {error}
            </div>)}

          <button onClick={analyze} disabled={analyzing || (!!status && !status.can_analyze)} className="btn btn-primary w-full sm:w-auto">
        ...
        <div>
          ...
          {history.length === 0 ? (<div className="rounded-[var(--radius-md)] border border-dashed p-6 text-center text-sm" style={{ borderColor: 'var(--line)', color: 'var(--muted)' }}>
              No analyses yet.
            </div>) : (<div className="panel divide-y overflow-hidden" style={{ borderColor: 'var(--line)' }}>
              {history.map((h) => (<div key={h.id} className="flex items-center justify-between gap-3 p-4">
                  <div>
      ...
      {result && (<div ref={resultRef} className="mt-12">
          ...
          {result.ai_feedback?.overall_feedback && (<p className="mt-4 max-w-2xl text-sm leading-relaxed" style={{ color: 'var(--ink-soft)' }}>
              {result.ai_feedback.overall_feedback}
            </p>)}

          {(result.ai_feedback?.headline_rewrite || result.ai_feedback?.about_rewrite) && (<div className="mt-6 grid gap-4 sm:grid-cols-2">
              {result.ai_feedback.headline_rewrite && (<div className="panel p-5">
              ...
              {result.ai_feedback.about_rewrite && (<div className="panel p-5">
          ...
          {result.ai_feedback?.priority_tips?.length > 0 && (<div className="mt-8">
              <p className="eyebrow eyebrow-accent mb-3">// priority fixes</p>
              <ol className="space-y-2">
                {result.ai_feedback.priority_tips.map((tip, i) => (<li key={i} className="panel p-4 text-sm leading-relaxed">
          ...
          <div className="mt-8">
            ...
            <div className="panel divide-y overflow-hidden" style={{ borderColor: 'var(--line)' }}>
              {result.rules.map((r) => (<div key={r.id} className="flex items-start justify-between gap-3 p-4">
                  <div className="min-w-0 flex-1">
                    <p className="text-sm font-medium">{r.label}</p>
                    <p className="mt-0.5 text-xs" style={{ color: 'var(--ink-soft)' }}>{r.detail}</p>
                    {!r.passed && (<p className="mt-1 text-xs" style={{ color: 'var(--muted)' }}>{r.tip}</p>)}
                  </div>
                  <span className="flex-shrink-0 text-xs font-semibold" style={{ color: r.passed ? 'var(--green)' : 'var(--rust)' }}>
                    {r.passed ? 'Pass' : 'Fix'}
                  </span>
                </div>))}
            </div>
