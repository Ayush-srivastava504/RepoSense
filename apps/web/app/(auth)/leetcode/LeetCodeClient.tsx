// Module: app/(auth)/leetcode/LeetCodeClient.tsx
// Defines component(s)/export(s): DIFFICULTY_ORDER, STARRED_KEY, LeetCodeContent, LeetCodeClient
// Defines function(s): difficultyColor, progressKey, loadSet, saveSet
// Defines type(s): LevelSummary, LevelProblem, LevelDetail
...
function LeetCodeContent() {
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                ...
                setLevels(data);
                if (data.length > 0)
                    setActiveLevel(data[0].key);
            }
            catch (e: any) {
                if (!cancelled)
                    setError(e?.message || 'Could not load LeetCode levels.');
            }
            finally {
                if (!cancelled)
                    setLoading(false);
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                ...
                if (!cancelled)
                    setError(e?.message || 'Could not load this level.');
            }
            finally {
                if (!cancelled)
    ...
    return (<div className="container-xl py-12">
      ...
      {error && (<p className="mt-6" style={{ color: 'var(--rust)', fontSize: '0.8125rem' }} role="alert">
          {error}
        </p>)}
      ...
      {loading ? (<p className="mt-8 text-sm" style={{ color: 'var(--ink-soft)' }}>Loading levels…</p>) : (<>
          ...
          {detail && (<p className="mt-4 text-xs" style={{ color: 'var(--ink-soft)' }}>
              {detail.description}
            </p>)}
          ...
          {detail && (<div className="mt-4">
              ...
              <div className="mt-1.5 h-2 w-full overflow-hidden rounded-full" style={{ backgroundColor: 'var(--line)' }}>
                <div className="h-full rounded-full transition-all" style={{ width: `${progressPct}%`, backgroundColor: 'var(--green)' }}/>
              </div>
            </div>)}
            ...
            {!detailLoading && filteredProblems.map((p) => {
                ...
                const Row = (<div className="flex items-center justify-between gap-3 rounded-[var(--radius-sm)] border px-4 py-3 transition-colors" style={{ borderColor: 'var(--line)' }}>
                  <div className="flex min-w-0 items-center gap-3">
                    ...
                    <span aria-hidden className="inline-flex h-4 w-4 flex-shrink-0 items-center justify-center rounded-full border text-[10px]" style={{
                    ...
                    <div className="min-w-0">
                      ...
                      {p.companies.length > 0 ? (<span className="mt-0.5 block truncate text-[11px]" style={{ color: 'var(--ink-soft)' }}>
                          {visibleCompanies.join(' · ')}
                          {extraCount > 0 && ` · +${extraCount} more`}
                        </span>) : (!p.solvable && (<span className="mt-0.5 inline-block rounded-full border px-2 py-0.5 text-[10px]" style={{ borderColor: 'var(--line)', color: 'var(--ink-soft)' }}>
                            on LeetCode
                          </span>))}
                    </div>
                  </div>
                ...
                return p.solvable ? (<Link key={p.slug} href={`/leetcode/${p.slug}`} className="block hover:opacity-80">
                  {Row}
                </Link>) : (<a key={p.slug} href={p.leetcode_url} target="_blank" rel="noopener noreferrer" className="block hover:opacity-80" onClick={() => trackEvent('leetcode_external_open', { slug: p.slug })}>
                  {Row}
                </a>);
            })}
          </div>
