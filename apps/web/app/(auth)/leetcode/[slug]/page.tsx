...
// Same fallback chain as lib/hackathons.ts / lib/companies.ts — some
// environments only ever set NEXT_PUBLIC_API_BASE_URL, not the
...
async function getProblem(slug: string): Promise<ProblemSummary | null> {
  try {
    const res = await fetch(
      `${API_BASE_URL}/api/leetcode/problems/${encodeURIComponent(slug)}`,
...
export async function generateMetadata({
  ...
  const description = `Solve "${problem.title}", a ${problem.difficulty.toLowerCase()} LeetCode problem, right in your browser against real test cases. ${truncate(
    problem.description,
    120,
  )}`;
