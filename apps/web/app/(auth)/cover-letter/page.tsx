// Module: app/(auth)/cover-letter/page.tsx
// Defines component(s)/export(s): CoverLetterContent, CoverLetterPage
//
//
      ...
      <div className="mt-8 grid gap-8 lg:grid-cols-[minmax(0,1fr)_1fr]">
        <div className="space-y-4">
          <div>
            ...
            <input className="field" placeholder="e.g. Razorpay" value={companyName} onChange={(e) => setCompanyName(e.target.value)}/>
          </div>

          <div>
            <label className="field-label">Job description</label>
            <textarea value={jobDescription} onChange={(e) => setJobDescription(e.target.value)} placeholder="Paste the internship or job posting here…" rows={8} className="field font-mono text-xs leading-relaxed"/>
          </div>

          <div>
            <label className="field-label">Your resume text</label>
            <textarea value={resumeText} onChange={(e) => setResumeText(e.target.value)} placeholder="Paste your resume text here…" rows={8} className="field font-mono text-xs leading-relaxed"/>
          </div>
        ...
        <div>
          {!letter && !loading && (<div className="rounded-[var(--radius-md)] border p-6 text-sm" style={{ borderColor: 'var(--line)', color: 'var(--ink-soft)' }}>
              Your generated letter will show up here — editable, ready to copy into an application.
            </div>)}

          {loading && (<div className="rounded-[var(--radius-md)] border p-6 text-sm" style={{ borderColor: 'var(--line)', color: 'var(--ink-soft)' }}>
              Writing a draft based on your resume and the job description… this can take a minute or two.
            </div>)}

          {letter && (<div className="space-y-3">
              ...
              <textarea value={letter} onChange={(e) => setLetter(e.target.value)} rows={20} className="field text-sm leading-relaxed"/>
            </div>)}
        </div>
      </div>
    </div>);
