...
// Defines component(s)/export(s): StatCard, ScoreBadge, SearchIcon, SkeletonRow, EmptyState, SectionHeader
// Defines function(s): scoreColor, greeting
// Defines type(s): Stats, RecentReview, RecentResume, ConnectedRepo
...
function StatCard({ label, value, sub, accent, }: {
    ...
    return (<div className="panel p-5">
      ...
      <p className="display text-3xl font-medium tabular-nums" style={{ color: accent ? colorMap[accent] : 'var(--ink)' }}>
        {value}
      </p>
      {sub && (<p className="mt-1 text-xs" style={{ color: 'var(--muted)' }}>
          {sub}
        </p>)}
    </div>);
}
function ScoreBadge({ score }: {
    ...
    return (<span className="flex-shrink-0 tabular-nums text-sm font-semibold" style={{ color: scoreColor(score) }}>
      {score}/100
    </span>);
}
function SearchIcon() {
...
function SkeletonRow() {
    return (<div className="flex items-center justify-between p-4 animate-pulse">
...
function EmptyState({ label, cta, href }: {
    label: string;
    cta: string;
    href: string;
}) {
    return (<div className="flex flex-col items-center justify-center rounded-[var(--radius-md)] border py-10 px-6 text-center" style={{ borderColor: 'var(--line)', borderStyle: 'dashed' }}>
...
function SectionHeader({ label, linkLabel, linkHref, }: {
    label: string;
    linkLabel: string;
    linkHref: string;
}) {
...
function DashboardContent() {
    ...
    useEffect(() => {
        ...
        Promise.allSettled([
            ...
            if (resumesRes.status === 'fulfilled')
                setRecentResumes(resumesRes.value ?? []);
            if (reposRes.status === 'fulfilled')
                setRepos((reposRes.value ?? []).slice(0, 4));
            setLoadingActivity(false);
        });
    }, [user]);
    ...
    return (<>
      ...
      {user ? (<div className="mt-8 grid grid-cols-2 gap-3 lg:grid-cols-4">
          {loadingStats ? (Array.from({ length: 4 }).map((_, i) => (<div key={i} className="panel p-5 animate-pulse">
      ...
      {user && !loadingStats && newUser && (<div className="mt-10">
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            {[
                ...
                {!item.done && (<Link href={item.href} className="btn btn-secondary mt-4 w-full text-sm">
                    {item.cta}
                  </Link>)}
              </div>))}
          </div>
