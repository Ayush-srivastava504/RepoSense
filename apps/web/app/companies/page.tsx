// Module: app/companies/page.tsx
// Defines component(s)/export(s): Section, CompaniesPage
//
//
...
function Section({ id, eyebrow, title, description, companies, total, accent, }: {
    ...
    if (companies.length === 0)
        return null;
    return (<section id={id} className="py-8 sm:py-10">
      ...
      <div className="grid gap-3 sm:gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
        {companies.map((c) => (<CompanyCard key={c.company} company={c}/>))}
      </div>

      {total > companies.length && (<p className="mt-5 text-xs sm:text-sm px-0.5" style={{ color: 'var(--muted)' }}>
...
export default async function CompaniesPage() {
    ...
    return (<main className="mx-auto max-w-6xl px-3 sm:px-4 py-8 sm:py-12">
      ...
      <header>
        ...
        {totalCompanies > 0 && (<div className="mt-5 flex flex-wrap gap-2">
            {top.companies.length > 0 && (<a href="#top" className="chip chip-indigo text-xs touch-manipulation">
                Top Companies ({top.total})
              </a>)}
            {mass_hire.companies.length > 0 && (<a href="#mass-hire" className="chip chip-green text-xs touch-manipulation">
                Mass Hiring ({mass_hire.total})
              </a>)}
            {startup.companies.length > 0 && (<a href="#startups" className="chip chip-rust text-xs touch-manipulation">
                Startups ({startup.total})
              </a>)}
          </div>)}
      </header>

      <Section id="top" eyebrow="top companies" title="Top Companies" description="Established names — big tech, IT services, BFSI, and well-known consumer brands — actively hiring right now." companies={top.companies} total={top.total} accent="var(--indigo)"/>

      <Section id="mass-hire" eyebrow="mass hiring" title="Mass Hiring" description={`Companies currently running ${mass_hire_threshold}+ open listings at once — a genuine hiring drive, not a single opening.`} companies={mass_hire.companies} total={mass_hire.total} accent="var(--green)"/>

      <Section id="startups" eyebrow="startups" title="Startups" description="Smaller and newer companies with an active listing — a mix of early-stage startups and niche employers." companies={startup.companies} total={startup.total} accent="var(--rust)"/>
