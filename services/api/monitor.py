...
try:
    ...
    import httpx
    import aiofiles
SERVICES = [{'name': 'API Gateway', 'url': 'http://localhost:8000/health', 'critical': True}, {'name': 'Neural Generator', 'url': 'http://localhost:8002/health', 'critical': False}, {'name': 'RAG Service', 'url': 'http://localhost:8001/health', 'critical': False}, {'name': 'Crawler', 'url': 'http://localhost:8003/health', 'critical': False}]
TIMEOUT_SECONDS = 5
...
async def check_service(client: httpx.AsyncClient, svc: dict) -> dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            body = {}
        return {'service': svc['name'], 'status': 'up' if ok else 'degraded', 'http_code': resp.status_code, 'latency_ms': latency_ms, 'detail': body, 'critical': svc['critical']}
...
def _down_result(svc: dict, reason: str) -> dict[str, Any]:
    return {'service': svc['name'], 'status': 'down', 'http_code': None, 'latency_ms': None, 'detail': {'error': reason}, 'critical': svc['critical']}
...
async def run_checks() -> dict[str, Any]:
    ...
    overall_up = all((r['status'] == 'up' for r in results if r['critical']))
    report = {'timestamp': datetime.now(timezone.utc).isoformat(), 'overall': 'healthy' if overall_up else 'degraded', 'services': list(results)}
    for r in results:
        if r['status'] != 'up':
            await maybe_alert(r)
    return report
