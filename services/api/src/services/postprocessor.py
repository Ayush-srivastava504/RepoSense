# Module: src/services/postprocessor.py
# Defines class(es): Postprocessor
#
#
...
class Postprocessor:
    ...
    def process(self, issues: List[Dict[str, Any]], code_length: int) -> Dict[str, Any]:
        ...
        return {'issues': issues, 'quality_metrics': {'lines_of_code': code_length, 'total_issues': total, 'issue_density': round(total / max(code_length / 100, 1), 2), 'severity_breakdown': severity_breakdown, 'category_breakdown': category_breakdown, 'quality_score': score}, 'summary': self._build_summary(total, severity_breakdown, score)}
