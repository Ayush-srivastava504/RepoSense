...
# Defines class(es): ReviewService
#
#
...
class ReviewService:
    ...
    async def review_single(self, request: ReviewRequest) -> ReviewResponse:
        ...
        raw_issues: List[Dict[str, Any]] = await asyncio.to_thread(engine.analyze, request.code, request.language.value, request.focus_areas)
        processed = self._postprocessor.process(raw_issues, code_length)
        issues = self._build_issues(processed['issues'])
        quality_metrics = QualityMetrics(**processed['quality_metrics']) if request.include_metrics else None
        ...
        return ReviewResponse(request_id=request_id, code_length=code_length, issues_found=len(issues), issues=issues, quality_metrics=quality_metrics, summary=processed['summary'], processing_time_ms=elapsed_ms, model_version=settings.model.MODEL_NAME)

    async def review_batch(self, requests: List[ReviewRequest], concurrency: int=_BATCH_CONCURRENCY) -> Dict[str, Any]:
        semaphore = asyncio.Semaphore(concurrency)

        async def _guarded(req: ReviewRequest) -> ReviewResponse:
            async with semaphore:
                return await self.review_single(req)
        results = await asyncio.gather(*(_guarded(r) for r in requests), return_exceptions=True)
        ...
        return {'total_items': len(requests), 'successful': len(successful), 'failed': len(failed), 'results': successful, 'errors': failed}
    ...
    def _build_issues(raw: List[Dict[str, Any]]) -> List[Issue]:
        ...
        for item in raw:
            try:
                issues.append(Issue(line=item.get('line'), column=item.get('col'), severity=Severity(item['severity']), category=item['category'], message=item['message'], suggestion=item['suggestion'], confidence=item['confidence']))
