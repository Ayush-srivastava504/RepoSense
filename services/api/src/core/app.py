...
# Defines function(s): lifespan, create_application
#
#
...
def create_application() -> FastAPI:
    app = FastAPI(title='Internship Platform API (with AI Review & Self-Healing)', description='Backend with code review, GitHub integration, resumes, jobs', version='2.0.0', docs_url='/docs', redoc_url='/redoc', lifespan=lifespan)
    ...
    async def request_logging_middleware(request: Request, call_next):
        ...
        logger.info(json.dumps({'event': 'request', 'id': req_id, 'method': request.method, 'path': request.url.path, 'ip': request.headers.get('X-Forwarded-For', request.client.host if request.client else 'unknown')}))
        start = time.time()
        response = await call_next(request)
        ms = round((time.time() - start) * 1000, 1)
        logger.info(json.dumps({'event': 'response', 'id': req_id, 'status': response.status_code, 'ms': ms}))
        return response
    ...
    @app.get('/', tags=['meta'])
    async def root():
        return {'service': 'Internship Platform API', 'version': '2.0.0', 'status': 'operational', 'docs': '/docs'}
