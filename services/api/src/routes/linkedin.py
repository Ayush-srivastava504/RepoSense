...
async def analyze_profile(data: ProfileInput, user=Depends(verify_token)):
    ...
    if unlock_method is None:
        ...
        raise HTTPException(status_code=402, detail={'error': 'upgrade_required', 'message': "You've used your free LinkedIn optimization check. Upgrade to Pro or watch a quick ad to unlock another one.", 'status': status})
    job_id = await create_job(user_id=user['sub'], job_type='linkedin', payload={'unlock_method': unlock_method, 'profile': data.model_dump()})
    asyncio.create_task(run_linkedin_job(job_id=job_id, user_id=user['sub'], unlock_method=unlock_method, profile=data.model_dump()))
