# Module: src/routes/github.py
# Defines function(s): github_login, github_callback, exchange_code, disconnect_github, get_repos, _get_github_token, get_repo_contents, get_file_content
#
#
...
async def github_login(request: Request):
    ...
    params = {'client_id': settings.GITHUB_CLIENT_ID, 'redirect_uri': settings.GITHUB_REDIRECT_URI, 'scope': 'repo user', 'state': state}
    await redis.setex(f'github_oauth_state:{state}', 600, 'pending')
    return RedirectResponse('https://github.com/login/oauth/authorize?' + urlencode(params), status_code=302)
...
async def github_callback(request: Request, code: str, state: str):
    ...
    async with httpx.AsyncClient(timeout=timeout) as client:
        resp = await client.post('https://github.com/login/oauth/access_token', data={'client_id': settings.GITHUB_CLIENT_ID, 'client_secret': settings.GITHUB_CLIENT_SECRET, 'code': code, 'redirect_uri': settings.GITHUB_REDIRECT_URI}, headers={'Accept': 'application/json'})
        data = resp.json()
        ...
        if not email:
            emails_resp = await client.get('https://api.github.com/user/emails', headers={'Authorization': f'token {access_token}'})
    ...
    await pool.execute('UPDATE users SET github_token = $1 WHERE id = $2', encrypted_token, user_row['id'])
    jwt_token = jwt.encode({'sub': str(user_row['id']), 'email': user_row['email'], 'subscription_tier': user_row['subscription_tier'], 'exp': datetime.utcnow() + timedelta(days=7)}, settings.JWT_SECRET, algorithm='HS256')
    one_time_code = secrets.token_urlsafe(32)
    await redis.setex(f'auth_code:{one_time_code}', 60, jwt_token)
    return RedirectResponse(f'{settings.FRONTEND_URL}/github?code={one_time_code}', status_code=302)
...
async def disconnect_github(user=Depends(verify_token)):
    ...
    if pool is None:
        raise HTTPException(503, 'Database unavailable')
    await pool.execute('UPDATE users SET github_token = NULL WHERE id = $1', user['sub'])
...
async def get_repos(user=Depends(verify_token)):
    ...
    return [{'id': repo['id'], 'full_name': repo['full_name'], 'private': repo.get('private', False)} for repo in repos]

async def _get_github_token(user_id: str, pool) -> str:
    user_row = await pool.fetchrow('SELECT github_token FROM users WHERE id = $1', user_id)
    if not user_row or not user_row['github_token']:
        raise HTTPException(401, 'GitHub not connected')
...
async def get_repo_contents(repo: str, path: str='', user=Depends(verify_token)):
    ...
    async with httpx.AsyncClient(timeout=httpx.Timeout(60.0, connect=30.0)) as client:
        resp = await client.get(f'https://api.github.com/repos/{repo}/contents/{path}', headers={'Authorization': f'token {github_token}', 'Accept': 'application/vnd.github.v3+json'})
...
async def get_file_content(repo: str, path: str, user=Depends(verify_token)):
    ...
    github_token = await _get_github_token(user['sub'], pool)
    async with httpx.AsyncClient(timeout=httpx.Timeout(60.0, connect=30.0)) as client:
        resp = await client.get(f'https://api.github.com/repos/{repo}/contents/{path}', headers={'Authorization': f'token {github_token}', 'Accept': 'application/vnd.github.v3.raw'})
        resp.raise_for_status()
...
async def auto_setup(owner: str, repo: str, user=Depends(verify_token)):
    ...
    job_id = await create_job(user_id=user['sub'], job_type='readme', payload={'repo': repo_name})
    asyncio.create_task(run_readme_job(job_id=job_id, user_id=user['sub'], github_token=github_token, repo_name=repo_name))
