...
class PreprocessingConfig(BaseSettings):
    MAX_CODE_LENGTH: int = Field(
        default=10000,
        env="PREPROCESS_MAX_CODE_LENGTH",
    )

    CHUNK_SIZE: int = Field(
        default=500,
        env="PREPROCESS_CHUNK_SIZE",
    )

    REMOVE_COMMENTS: bool = Field(
        default=True,
        env="PREPROCESS_REMOVE_COMMENTS",
    )

    NORMALIZE_WHITESPACE: bool = Field(
        default=True,
        env="PREPROCESS_NORMALIZE_WHITESPACE",
    )
...
class InferenceConfig(BaseSettings):
    CONFIDENCE_THRESHOLD: float = Field(
        default=0.7,
        env="INFERENCE_CONFIDENCE_THRESHOLD",
    )

    TOP_K_ISSUES: int = Field(
        default=20,
        env="INFERENCE_TOP_K_ISSUES",
    )
...
class SecurityConfig(BaseSettings):
    API_KEY_ENABLED: bool = Field(
        default=False,
        env="SECURITY_API_KEY_ENABLED",
    )

    API_KEY: Optional[str] = Field(
        default=None,
        env="SECURITY_API_KEY",
    )

    MAX_REQUEST_SIZE: int = Field(
        default=10 * 1024 * 1024,
        env="SECURITY_MAX_REQUEST_SIZE",
    )

    RATE_LIMIT_REQUESTS: int = Field(
        default=100,
        env="SECURITY_RATE_LIMIT_REQUESTS",
    )

    RATE_LIMIT_PERIOD: int = Field(
        default=60,
        env="SECURITY_RATE_LIMIT_PERIOD",
    )
...
class MLSettings:
    def __init__(self):
        ...
        self.GITHUB_TOKEN_ENCRYPTION_KEY = os.getenv(
