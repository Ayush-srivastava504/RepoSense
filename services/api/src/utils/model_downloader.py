# Module: src/utils/model_downloader.py
# Defines class(es): ModelDownloader
# Defines function(s): get_downloader
#
...
class ModelDownloader:
    ...
    def download_codebert(self, model_name: str='microsoft/codebert-base') -> Dict[str, Any]:
        try:
            ...
            model = AutoModel.from_pretrained(model_name, cache_dir=str(self.cache_dir), trust_remote_code=True)
            tokenizer = AutoTokenizer.from_pretrained(model_name, cache_dir=str(self.cache_dir), trust_remote_code=True)
            logger.info(f'Successfully loaded CodeBERT: {model_name}')
            return {'type': 'codebert', 'name': model_name, 'model': model, 'tokenizer': tokenizer, 'cache_dir': str(self.cache_dir), 'success': True}
        except Exception as e:
            logger.error(f'Failed to download CodeBERT: {e}')
            return {'type': 'codebert', 'success': False, 'error': str(e)}

    def download_qwen_gguf(self, repo_id: str='TheBloke/Qwen2-0.5B-Instruct-GGUF', filename: str='qwen2-0.5b-instruct.Q4_K_M.gguf') -> Dict[str, Any]:
        try:
            from huggingface_hub import hf_hub_download
            logger.info(f'Downloading Qwen model: {repo_id}/{filename}')
            model_path = hf_hub_download(repo_id=repo_id, filename=filename, cache_dir=str(self.cache_dir))
            logger.info(f'Successfully downloaded Qwen model to: {model_path}')
            return {'type': 'qwen_gguf', 'repo_id': repo_id, 'filename': filename, 'local_path': str(model_path), 'success': True}
        except Exception as e:
            logger.error(f'Failed to download Qwen GGUF: {e}')
            return {'type': 'qwen_gguf', 'success': False, 'error': str(e)}

    def download_custom_model(self, repo_id: str, filename: Optional[str]=None, model_type: str='transformer') -> Dict[str, Any]:
        try:
            if model_type == 'gguf':
                if not filename:
                ...
                return {'type': model_type, 'repo_id': repo_id, 'filename': filename, 'local_path': str(model_path), 'success': True}
            elif model_type == 'transformer':
                from transformers import AutoModel, AutoTokenizer
                model = AutoModel.from_pretrained(repo_id, cache_dir=str(self.cache_dir), trust_remote_code=True)
                tokenizer = AutoTokenizer.from_pretrained(repo_id, cache_dir=str(self.cache_dir), trust_remote_code=True)
                logger.info(f'Downloaded transformer model: {repo_id}')
                return {'type': 'transformer', 'repo_id': repo_id, 'model': model, 'tokenizer': tokenizer, 'cache_dir': str(self.cache_dir), 'success': True}
            else:
    ...
    def get_cached_models(self) -> Dict[str, list]:
        ...
        cached['transformers'] = sorted({folder.parent.parent.name for folder in self.cache_dir.glob('models--*/snapshots/*/') if folder.exists()})
...
def get_downloader(cache_dir: Optional[str]=None) -> ModelDownloader:
    ...
    if _downloader is None:
        ...
        warnings.warn('Singleton already exists — cache_dir argument ignored. Instantiate ModelDownloader directly to use a different path.', UserWarning, stacklevel=2)
    return _downloader
