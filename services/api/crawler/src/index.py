# Module: crawler/src/index.py
# Defines function(s): _load_scrapers, run_pipeline, _run_scraper, lambda_handler
#
#
...
def _load_scrapers() -> Dict:
    registry: Dict = {}
    try:
        from scrapers.internshala import InternshalaScaper
        registry['internshala'] = InternshalaScaper
    except ImportError as exc:
        log.warning('internshala scraper unavailable: %s', exc)
    try:
        from scrapers.linkedin import LinkedInScraper
        registry['linkedin'] = LinkedInScraper
    except ImportError as exc:
        log.warning('linkedin scraper unavailable: %s', exc)
    try:
        from scrapers.hiringcafe import HiringCafeScraper
        registry['hiringcafe'] = HiringCafeScraper
    except ImportError as exc:
        log.warning('hiringcafe scraper unavailable: %s', exc)
    try:
        from scrapers.unstop import UnstopScraper
        registry['unstop'] = UnstopScraper
    except ImportError as exc:
        log.warning('unstop scraper unavailable: %s', exc)
    try:
        from scrapers.cutshort import CutshortScraper
        registry['cutshort'] = CutshortScraper
    except ImportError as exc:
        log.warning('cutshort scraper unavailable: %s', exc)
    try:
        from scrapers.company_portals import CompanyPortalsScraper
        registry['company_portals'] = CompanyPortalsScraper
    except ImportError as exc:
        log.warning('company_portals scraper unavailable: %s', exc)
    try:
        from scrapers.remoteok import RemoteOKScraper
        registry['remoteok'] = RemoteOKScraper
    except ImportError as exc:
        log.warning('remoteok scraper unavailable: %s', exc)
    try:
        from scrapers.weworkremotely import WeWorkRemotelyScraper
        registry['weworkremotely'] = WeWorkRemotelyScraper
    except ImportError as exc:
        log.warning('weworkremotely scraper unavailable: %s', exc)
    try:
        from scrapers.remotive import RemotiveScraper
        registry['remotive'] = RemotiveScraper
    except ImportError as exc:
        log.warning('remotive scraper unavailable: %s', exc)
    try:
        from scrapers.japan_jobs import JapanJobsScraper
        registry['japan_jobs'] = JapanJobsScraper
    except ImportError as exc:
        log.warning('japan_jobs scraper unavailable: %s', exc)
    try:
        from scrapers.japan_internships import JapanInternshipsScraper
        registry['japan_internships'] = JapanInternshipsScraper
    except ImportError as exc:
        log.warning('japan_internships scraper unavailable: %s', exc)
    _europe_scrapers = [('europe_jobicy', 'scrapers.europe_jobicy', 'EuropeJobicyScraper'), ('europe_arbeitnow', 'scrapers.europe_arbeitnow', 'EuropeArbeitnowScraper'), ('europe_remotive', 'scrapers.europe_remotive', 'EuropeRemotiveScraper'), ('europe_weworkremotely', 'scrapers.europe_weworkremotely', 'EuropeWeWorkRemotelyScraper'), ('europe_remoteok', 'scrapers.europe_remoteok', 'EuropeRemoteOKScraper')]
    ...
    try:
        ...
        registry['employment_news'] = EmploymentNewsScraper
    except ImportError as exc:
        log.warning('employment_news scraper unavailable: %s', exc)
    try:
    ...
    try:
        ...
        log.warning('generic_boards scraper unavailable: %s', exc)
    return registry

def run_pipeline(keywords: List[str]=None, locations: List[str]=None, max_pages: int=MAX_PAGES_PER_SOURCE, existing_ids: Set[str]=None, dry_run: bool=False) -> Dict:
    keywords = keywords or DEFAULT_KEYWORDS
    locations = locations or DEFAULT_LOCATIONS
    started_at = utcnow()
    started = time.time()
    registry = _load_scrapers()
    enabled_scrapers = [s.strip() for s in os.getenv('ENABLED_SCRAPERS', ','.join(ENABLED_SCRAPERS)).split(',') if s.strip()]
    active_scrapers = [scraper for scraper in enabled_scrapers if scraper in registry]
    log.info('Pipeline start | scrapers=%s | keywords=%d | locations=%d | max_pages=%d', active_scrapers, len(keywords), len(locations), max_pages)
    raw_jobs: List[Dict] = []
    source_counts: Dict[str, int] = {}
    for key in active_scrapers:
        try:
            scraper = registry[key]()
            jobs = _run_scraper(scraper, keywords, locations, max_pages)
            source_counts[key] = len(jobs)
            raw_jobs.extend(jobs)
            log.info('%s -> %d raw jobs', key, len(jobs))
        except Exception as exc:
            log.error('%s crashed: %s', key, exc, exc_info=True)
            source_counts[key] = 0
    ...
    if existing_ids:
        deduped = deduplicate_incremental(normalized, existing_ids)
    else:
...
def _run_scraper(scraper, keywords, locations, max_pages) -> List[Dict]:
    return scraper.run(keywords=keywords, locations=locations, max_pages=max_pages)

def lambda_handler(event: Dict, context) -> Dict:
    log.info('Lambda invoked | event=%s', json.dumps(event))
    if 'scrapers' in event:
        os.environ['ENABLED_SCRAPERS'] = ','.join([s.strip() for s in event['scrapers'] if s.strip()])
        import importlib
        import config
        importlib.reload(config)
    summary = run_pipeline(keywords=event.get('keywords'), locations=event.get('locations'), max_pages=int(event.get('max_pages', MAX_PAGES_PER_SOURCE)), dry_run=bool(event.get('dry_run', False)))
...
if __name__ == '__main__':
    ...
    if args.scrapers:
        ...
        for item in args.scrapers:
            scraper_list.extend([s.strip() for s in item.split(',') if s.strip()])
        os.environ['ENABLED_SCRAPERS'] = ','.join(scraper_list)
        log.info('Using scrapers: %s', os.environ['ENABLED_SCRAPERS'])
    summary = run_pipeline(keywords=args.keywords, locations=args.locations, max_pages=args.max_pages, dry_run=args.dry_run)
