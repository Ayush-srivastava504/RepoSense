...
ENABLED_SCRAPERS: List[str] = os.getenv('ENABLED_SCRAPERS', 'internshala,linkedin,hiringcafe,unstop,cutshort,company_portals,remoteok,weworkremotely,remotive,japan_jobs,japan_internships,europe_jobicy,europe_arbeitnow,europe_remotive,europe_weworkremotely,europe_remoteok,employment_news,freejobalert,greenhouse,lever,ashby,smartrecruiters,workable,recruitee,teamtailor,bamboohr,breezyhr,personio,freshteam,naukri,dorker,generic_boards').split(',')
# NOTE on ashby/smartrecruiters board lists: my sandbox has no general
# internet access (only package registries + GitHub are allowlisted), so
# these slugs are NOT live-verified — they're a best-effort refresh based
# on which companies are publicly known to use each ATS, swapped toward
# ones more likely to run intern/new-grad pipelines than the previous
# list. Any hardcoded board list will keep rotting as companies switch
# ATS providers or close boards; treat this as a stopgap, not a fix.
# The real fix (see FresherFlow's dorker.ts) is dynamic board discovery
# via search rather than a static company list — worth porting over as a
# follow-up rather than re-patching this list again next quarter.
#
# UPDATE (Phase 1): that follow-up now exists — see scrapers/dorker.py,
# which runs Bing-dork queries against greenhouse/lever/ashby board URLs
# every pipeline run and fetches jobs from whatever new boards it finds,
# skipping tokens already listed below. The static lists below are NOT
# being removed, though: dorker.py is additive discovery, not a
# replacement — it only searches a handful of dork queries per run and
# will miss boards that don't surface in the first page of Bing results,
# so these hand-maintained lists remain the reliable baseline. The
# recruitee/teamtailor/bamboohr/breezyhr/personio/freshteam token lists
# below carry the same live-verification caveat as ashby/smartrecruiters
# above — added for coverage breadth, not verified against each board's
# current existence.
ATS_COMPANIES: Dict[str, List[str]] = {'greenhouse': ['stripe', 'airbnb', 'coinbase', 'robinhood', 'gitlab', 'figma', 'discord', 'cloudflare', 'dropbox', 'asana', 'affirm', 'squarespace', 'pinterest', 'reddit', 'instacart', 'twitch', 'brex', 'databricks', 'snowflake', 'mongodb', 'hashicorp', 'confluent', 'elastic', 'twilio', 'roblox', 'unity', 'zscaler', 'crowdstrike', 'palantir', 'webflow', 'rippling', 'deel', 'vercel', 'scaleai', 'anthropic', 'duolingo', 'toast', 'carta', 'gusto', 'justworks', 'benchling', 'samsara', 'klaviyo', 'amplitude', 'sourcegraph', 'circleci', 'pagerduty', 'grafana-labs', 'okta', 'netskope'], 'lever': ['plaid', 'kraken', 'anchorage', 'wealthsimple', 'netflix', 'shopify'], 'ashby': ['notion', 'linear', 'vercel', 'ramp', 'mercury', 'substack', 'deel', 'airtable', 'retool', 'replit'], 'smartrecruiters': ['visa', 'ikea', 'skechers', 'bosch', 'yourfoodjob'], 'workable': ['huggingface', 'typeform', 'deelhq'], 'recruitee': ['picnic', 'framer', 'bynder', 'labguru', 'mollie'], 'teamtailor': ['deliverect', 'lokalise', 'storyblok', 'remote-com'], 'bamboohr': ['clevertap', 'chargebee', 'freshworks', 'zenoti'], 'breezyhr': ['clevertech', 'shipbob', 'invisionapp'], 'personio': ['personio', 'contentful', 'depop'], 'freshteam': ['freshworks', 'chargebee']}
