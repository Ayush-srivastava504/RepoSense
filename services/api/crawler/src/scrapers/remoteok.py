...
REQUEST_TIMEOUT = 30
DEFAULT_TAGS = {'dev', 'engineer', 'developer', 'software', 'data', 'backend', 'frontend', 'fullstack', 'full stack', 'python', 'javascript', 'react', 'node', 'java', 'golang', 'devops', 'ml', 'machine learning', 'product', 'design', 'intern'}
...
class RemoteOKScraper(BaseScraper):
    ...
    def scrape(self, keywords: List[str], locations: List[str], max_pages: int) -> List[Dict]:
        ...
        try:
            ...
            if not isinstance(data, list):
                self.log.warning('Remote OK unexpected JSON root: %s', type(data).__name__)
                return jobs
            entries = [row for row in data if isinstance(row, dict) and row.get('id')]
            self.log.info('Remote OK payload had %d rows, %d look like jobs', len(data), len(entries))
    ...
    def _fetch(self, session: requests.Session) -> Optional[list]:
        ...
        self.log.info('Remote OK HTTP %d content-type=%s bytes=%d url=%s', response.status_code, content_type, len(response.content), response.url)
        if response.status_code == 403:
            self.log.warning('Remote OK returned 403 — likely Cloudflare/bot block, body=%r', response.text[:300])
            return None
        if response.status_code != 200:
            self.log.warning('Remote OK HTTP failure %d body=%r', response.status_code, response.text[:300])
            return None
        ...
        if 'application/json' not in content_type:
            self.log.warning('Remote OK returned non-JSON content-type=%s body=%r', content_type, body[:300])
            return None
        try:
            return response.json()
        except ValueError as exc:
            self.log.warning('Remote OK JSON decode failed: %s body=%r', exc, body[:300])
            return None

    def _parse_entry(self, entry: Dict, keyword_set: Set[str]) -> Optional[Dict]:
        ...
        return {'title': title, 'company': company, 'location': location, 'type': 'full-time', 'salary': salary, 'description': entry.get('description'), 'skills': tags, 'apply_url': apply_url, 'posted_date': entry.get('date') or '', 'is_remote': True, 'country': location}
