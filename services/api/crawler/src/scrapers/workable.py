    ...
    def _fetch_account(self, session, account: str) -> List[Dict]:
        ...
        for entry in entries:
            ...
            if isinstance(loc_obj, dict):
                location = ', '.join((p for p in (loc_obj.get('city'), loc_obj.get('country')) if p))
            shortcode = entry.get('shortcode', '')
            apply_url = f'https://apply.workable.com/{account}/j/{shortcode}/' if shortcode else ''
            job = build_job(title=entry.get('title', ''), company=company_name, location=location, apply_url=apply_url, job_type=(entry.get('employment_type') or '').lower() or None, posted_date=str(entry.get('published_on', '') or ''), source=self.source_name)
            if job:
                out.append(job)
