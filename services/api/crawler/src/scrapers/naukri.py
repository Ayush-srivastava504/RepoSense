...
class NaukriScraper(BaseScraper):
    ...
    # ------------------------------------------------------------------
    # Strategy chain: direct API -> browser-captured API -> HTML fallback
    # ------------------------------------------------------------------

    def _search(self, keyword: str, location: str, page: int) -> List[Dict]:

        jobs = self._search_via_direct_api(keyword, location, page)
        if jobs:
            return jobs

        jobs = self._search_via_browser_capture(keyword, location, page)
        if jobs:
            self.log.info(
                "Naukri kw=%r page=%d: direct API gave nothing, "
                "browser-captured API response worked (%d jobs)",
                keyword,
                page,
                len(jobs),
            )
    ...
    def _search_via_direct_api(
        ...
        if not response:
            self.log.info(
                "Naukri direct API: no response object (request failed) "
                "for kw=%r page=%d",
                keyword,
                page,
            )
        ...
        if status != 200:
            self.log.info(
                "Naukri direct API: status=%s for kw=%r page=%d "
                "(likely blocked / needs updated auth token) body=%r",
                status,
                keyword,
                page,
        ...
        try:
            ...
            self.log.info(
                "Naukri direct API: 200 but non-JSON body for kw=%r page=%d: %r",
                keyword,
                page,
                response.text[:200] if hasattr(response, "text") else "",
            )
        ...
        if not raw_jobs:
            self.log.info(
                "Naukri direct API: 200 + valid JSON but 0 job entries for "
                "kw=%r page=%d (response shape may have changed, keys=%s)",
                keyword,
                page,
                list(data.keys()),
            )
    ...
    def _search_via_browser_capture(
        self, keyword: str, location: str, page: int
    ) -> List[Dict]:
        """
        Let Naukri's own front-end JS make the jobapi request (with whatever
        anti-scraping token/headers it currently requires) and just read the
        response off the wire, instead of trying to forge that request
        ourselves. This is more resilient to Naukri changing their signing
        scheme, since we never have to know what it is.
        """
        captured: List[dict] = []
        ...
        try:
            ...
            self.log.warning("Naukri browser launch error: %s", str(e))
            return []
        ...
        try:
            ...
            try:
                ...
                self.log.warning("Naukri navigation error: %s", str(e))
                return []
    ...
    def _parse_api(self, raw: Dict) -> Optional[Dict]:
        ...
        if isinstance(placeholders, dict):
            job["location"] = ", ".join(
                placeholders.get("location", "").split(",")[:3]
            )
        else:
        ...
        job["type"] = _infer_type(
            job["title"] + " " + job.get("experience_required", "")
        )
    ...
    def _scrape_html(self, keyword: str, location: str, page: int) -> List[Dict]:
        ...
        # Structural/data-testid style selectors first (more stable),
        # visual classnames kept as a last-ditch fallback.
        selectors = [
            "[data-job-id]",
            "article.jobTuple",
            ".cust-job-tuple",
        ...
        for card in cards:
            try:
                ...
                job["experience_required"] = _clean(
                    exp_el.get_text(strip=True) if exp_el else ""
                )
                job["description"] = _clean(card.get_text(" ", strip=True))
                job["skills"] = []
                job["posted_date"] = ""
                job["type"] = _infer_type(
                    job["title"] + " " + job["experience_required"]
                )
                job["is_remote"] = "remote" in job["location"].lower()
        ...
        if not results:
            ...
            self.log.warning(
                "Naukri HTML fallback: 0 jobs. url=%s title=%r body_snippet=%r",
                final_url,
                title,
                body_text,
            )
    ...
    def _parse_jsonld(self, json_ld: Dict) -> Optional[Dict]:
        ...
        job["company"] = _clean(
            json_ld.get("hiringOrganization", {}).get("name", "")
        )
        job["location"] = _clean(
        ...
        description = re.sub(r"<[^>]+>", " ", json_ld.get("description", ""))
...
def _infer_type(text: str) -> str:
    ...
    if "intern" in text:
        return "internship"
    if "0 year" in text or "fresher" in text:
        return "full-time"
    return "full-time"
