# Module: crawler/src/scrapers/internshala.py
# Defines class(es): InternshalaScaper
...
class InternshalaScaper(BaseScraper):
    ...
    def _scrape_category(self, category: str, keyword: str, location: str, max_pages: int, job_type: str) -> List[Dict]:
        ...
        for page_num in range(1, max_pages + 1):
            ...
            selectors = ['.individual_internship', '.internship_meta', '.container-fluid.individual_internship', '[class*="internship_meta"]', '.internship-card']
            cards = []
    ...
    def _parse_card(self, card, job_type: str) -> Optional[Dict]:
        job = self._empty_job()
        title_el = card.select_one('.profile h3') or card.select_one('.job-internship-name') or card.select_one('h3') or card.select_one('a')
        company_el = card.select_one('.company_name') or card.select_one('.company-name') or card.select_one('h4')
        ...
        duration_el = card.select_one('.other_detail_item') or card.select_one('.duration')
