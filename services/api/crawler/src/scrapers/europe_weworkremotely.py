...
from scrapers.europe_common import make_session, fetch_weworkremotely_europe_entries, parse_weworkremotely_entry
...
class EuropeWeWorkRemotelyScraper(BaseScraper):
    ...
    def scrape(self, keywords: List[str], locations: List[str], max_pages: int) -> List[Dict]:
        ...
        try:
            entries = fetch_weworkremotely_europe_entries(session=session, log=self.log)
            for entry in entries:
                try:
                    job = parse_weworkremotely_entry(entry)
                except Exception as exc:
                    self.log.debug('Europe/WeWorkRemotely parse failed: %s', exc)
                    continue
