...
class DevfolioScraper(BaseHackathonScraper):
    ...
    def scrape(
        ...
        for listing_url in (
            ...
            try:
                ...
                self.log.exception(
                    "Failed scraping Devfolio listing"
                )
        ...
        self.log.info(
            "Collected %d hackathons from devfolio",
            len(results),
        )
    ...
    def _parse_card(
        ...
        hackathon["is_student_friendly"] = (
            _is_student_friendly(full_text)
        )
...
def _extract_location(
    ...
    match = re.search(
        r"Happening\s+(.+?)(?:Applications|Runs from|$)",
        text,
        re.IGNORECASE,
    )
