...
class UnstopHackathonScraper(BaseHackathonScraper):
    """Scrapes unstop.com/hackathons — a dedicated hackathon listing page,
    separate from the generic 'competitions' category the job crawler's
    UnstopScraper pulls into `jobs`."""
    ...
    def _parse_card(self, card) -> Optional[Dict]:
        ...
        prize_match = re.search(r"(₹|\$)\s?[\d,]+(\.\d+)?\s?(lakh|k|cr)?", text, re.IGNORECASE)
        h["prize_pool_text"] = prize_match.group(0) if prize_match else None

        deadline_match = re.search(
            r"(\d+\s?(days|hrs|hours)\s?(left|to go))", text, re.IGNORECASE
        )
...
THEME_KEYWORDS = [
    "ai", "machine learning", "ml", "web3", "blockchain", "open source",
    "healthcare", "fintech", "climate", "sustainability", "iot",
    "cybersecurity", "developer tools", "gaming", "edtech", "robotics",
]
