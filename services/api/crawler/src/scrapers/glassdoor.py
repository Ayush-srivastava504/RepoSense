...
class GlassdoorScraper(BaseScraper):
    ...
    def scrape(self, keywords: List[str], locations: List[str], max_pages: int) -> List[Dict]:
        jobs: List[Dict] = []
        keyword_queries = ["intern", "fresher", "graduate trainee", "junior software engineer"]
    ...
    def _search(self, keyword: str, max_pages: int) -> List[Dict]:
        ...
        for page in range(1, max_pages + 1):
            ...
            if os.getenv("SCRAPER_DEBUG"):
                with open(f"glassdoor_debug_{page}.html", "w", encoding="utf-8") as f:
                    f.write(html)
    ...
    def _parse_card(self, card) -> Optional[Dict]:
        ...
        title_el = (
            ...
            or card.select_one('[class*="jobTitle" i]')
            or card.select_one("h2")
            or card.select_one("a")
        )
        company_el = card.select_one('[data-test="employer-name"]') or card.select_one('[class*="employerName" i]')
        location_el = card.select_one('[data-test="location"]') or card.select_one('[class*="location" i]')
        salary_el = card.select_one('[data-test="detailSalary"]') or card.select_one('[class*="salaryEstimate" i]')
