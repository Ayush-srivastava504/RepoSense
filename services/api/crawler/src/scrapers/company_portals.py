# Module: crawler/src/scrapers/company_portals.py
# Defines class(es): CompanyPortalsScraper
# Defines function(s): _clean, _text, _infer_type, _map_employment_type, _salary_from_jsonld, _extract_items, _has_link, _find_link
#
...
TAG_RE = re.compile('<[^>]+>')
JOB_KEYWORDS = frozenset(('intern', 'trainee', 'engineer', 'developer', 'analyst', 'manager', 'associate', 'specialist', 'consultant', 'executive', 'lead', 'designer', 'scientist', 'architect', 'officer', 'coordinator', 'graduate', 'fresher'))
JOB_HREF_KEYWORDS = ('/job/', '/jobs/', '/career/', '/careers/', '/opening/', '/openings/', '/position/', '/positions/', '/vacancy/', '/vacancies/', 'requisition', 'jobid=', 'job_id=', 'job-id=', '/req/')
NON_JOB_TITLES = frozenset(('sign in', 'signin', 'log in', 'login', 'sign up', 'signup', 'register', 'create account', 'my account', 'profile', 'dashboard', 'home', 'about', 'contact', 'privacy', 'privacy policy', 'terms', 'terms of use', 'cookie policy', 'help', 'support', 'faq', 'search', 'search jobs', 'view jobs', 'all jobs', 'job search', 'careers', 'career', 'join us', 'learn more', 'read more', 'apply', 'apply now', 'view details', 'view job', 'view position', 'back', 'next', 'previous', 'menu', 'close'))
NON_JOB_TEXT_KEYWORDS = ('forgot password', 'remember me', 'create an account', 'already have an account', "don't have an account", 'accept cookies', 'cookie preferences')
CARD_SELECTOR_CANDIDATES = ['article', '.job-card', '.opening', '.job-listing', '.job-item', '.job-row', '.job-tile', '.position', '.posting', '.vacancy', 'li.job', 'tr.job', '[class*="job-card"]', '[class*="jobCard"]', '[class*="job-item"]', '[class*="jobItem"]', '[class*="job-listing"]', '[class*="posting"]', '[data-testid*="job"]', '[data-automation-id*="job"]', '[class*="opening"]', '[class*="vacan"]', '[class*="career"]', '[class*="job"]']
EMPLOYMENT_TYPE_MAP = {'FULL_TIME': 'full-time', 'Full-Time': 'full-time', 'Internship': 'internship', 'INTERN': 'internship', 'PART_TIME': 'part-time', 'TEMPORARY': 'contract', 'CONTRACT': 'contract'}
...
def _looks_like_job_url(url: str) -> bool:
    ...
    if not lowered or lowered.startswith(('#', 'javascript:', 'mailto:', 'tel:')):
        return False
    auth_markers = ('signin', 'sign-in', 'login', 'log-in', 'signup', 'sign-up', 'register', 'account', 'auth', 'sso')
...
def _is_valid_job(job: Dict) -> bool:
    ...
    context_keywords = ('responsibilit', 'qualification', 'requirement', 'experience', 'skills', 'role', 'employment', 'internship', 'full-time', 'part-time', 'location')
    return len(description) >= 80 and any((word in description for word in context_keywords))

def _dedupe_key(job: Dict) -> Tuple[str, str]:
    return ((job.get('title') or '').lower(), (job.get('apply_url') or '').lower())
...
class CompanyPortalsScraper(BaseScraper):
    ...
    def _search_html(self, company_key: str, config: Dict, max_pages: int) -> List[Dict]:
        ...
        for page_num in range(1, page_limit + 1):
            ...
            self.log.info('%s page %d: %d jsonld, %d cards via %s', config['name'], page_num, len(page_jobs), len(cards), used_selector)
            for card in cards:
                try:
    ...
    def _parse_html_card(self, card, config: Dict, selectors: Dict) -> Optional[Dict]:
        job = self._new_job(config)
        title_selectors = [selectors.get('title', ''), 'h2', 'h3', 'h4', '.title', '[class*="title"]', 'a']
        title = ''
    ...
    def _parse_api_item(self, raw: Dict, config: Dict) -> Optional[Dict]:
        ...
        if not job['title']:
            return None
        job['location'] = _clean(raw.get('location', '') or raw.get('city', '') or raw.get('locationName', '') or raw.get('primaryLocation', ''))
        job['description'] = _clean(TAG_RE.sub(' ', str(raw.get('description', '') or '')))
        job['salary'] = str(raw.get('salary', '') or raw.get('ctc', '') or '')
        job['apply_url'] = raw.get('url', '') or raw.get('apply_url', '') or raw.get('jobUrl', '') or raw.get('applyURL', '')
