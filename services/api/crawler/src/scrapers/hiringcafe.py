...
SEARCH_PAGE = f'{BASE}/'
API_HINTS = ('search-jobs', 'searchState')
API_HEADERS = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/124.0.0.0 Safari/537.36', 'Accept': 'application/json, text/plain, */*', 'Content-Type': 'application/json', 'Referer': f'{BASE}/', 'Origin': BASE}

def _build_search_state(keyword: str) -> Dict:
    return {'locations': [], 'workplaceTypes': ['Remote'], 'defaultToUserLocation': False, 'userLocation': None, 'currency': {'label': 'Any', 'value': None}, 'frequency': {'label': 'Any', 'value': None}, 'commitmentTypes': ['Full Time', 'Part Time', 'Contract', 'Internship'], 'jobTitleQuery': '', 'jobDescriptionQuery': '', 'seniorityLevel': ['No Prior Experience Required', 'Entry Level', 'Mid Level'], 'roleTypes': ['Individual Contributor'], 'searchQuery': keyword, 'dateFetchedPastNDays': 30, 'hiddenCompanies': [], 'user': None, 'searchModeSelectedCompany': None, 'departments': [], 'restrictedSearchAttributes': [], 'sortBy': 'date', 'technologyKeywordsQuery': '', 'requirementsKeywordsQuery': '', 'companyPublicOrPrivate': 'all', 'companySizeRanges': []}
...
class HiringCafeScraper(BaseScraper):
    ...
    def _fetch_via_api(self, keyword: str) -> List[Dict]:
        ...
        resp = safe_post(self.session, SEARCH_JOBS_ENDPOINT, json_data={'size': 100, 'page': 0, 'searchState': search_state}, headers=API_HEADERS, domain_key='hiringcafe')
        if resp is None:
            return []
        try:
            data = resp.json()
    ...
    def _fetch_via_browser(self, keyword: str) -> List[Dict]:
        ...
        if jobs:
            self.log.info('HiringCafe: got %d jobs from intercepted network responses', len(jobs))
            return jobs
    ...
    def _from_html_structure(self, soup) -> List[Dict]:
        link_pattern = re.compile('/(job|jobs|apply)/[^/?#]+', re.I)
    ...
    def _normalize(self, raw: Dict) -> Optional[Dict]:
        title = _clean(raw.get('title') or raw.get('job_title') or raw.get('jobTitle') or raw.get('role_title') or '')
        if not title:
            return None
        ...
        location = raw.get('location') or raw.get('formatted_location') or raw.get('locationName') or 'Remote'
        if isinstance(location, dict):
            location = location.get('name', '') or location.get('city', '') or 'Remote'
        location = _clean(str(location)) or 'Remote'
        workplace_type = str(raw.get('workplace_type') or raw.get('workplaceType') or '').lower()
        is_remote = 'remote' in workplace_type or 'remote' in location.lower() or bool(raw.get('is_remote'))
        ...
        description = re.sub('<[^>]+>', ' ', str(raw.get('description') or raw.get('job_description') or ''))
        apply_url = raw.get('apply_url') or raw.get('applyUrl') or raw.get('url') or raw.get('job_url') or ''
...
def _find_job_like_dicts(data, _depth: int=0) -> List[dict]:
    ...
    if isinstance(data, dict):
        ...
        support_keys = {'company', 'company_name', 'employer', 'location', 'formatted_location', 'salary', 'compensation', 'description', 'job_description', 'apply_url', 'applyUrl'}
        has_title = any((_clean(str(data.get(k) or '')) for k in title_keys))
