...
REQUEST_TIMEOUT = 30
JAPAN_HINTS = ('japan', 'tokyo', 'osaka', 'kyoto', 'yokohama', 'nagoya', 'fukuoka', 'sapporo', 'kobe', 'hiroshima', 'sendai', 'jp')

def fetch_jobicy_japan_entries(session: requests.Session, log) -> List[Dict]:
    try:
        response = session.get(JOBICY_API_URL, params={'count': 100, 'geo': 'japan'}, timeout=REQUEST_TIMEOUT, allow_redirects=True)
    ...
    if not isinstance(jobs, list):
        log.warning('Japan/Jobicy invalid jobs field: %s', type(jobs).__name__)
        return []
...
def parse_jobicy_entry(entry: Dict) -> Optional[Dict]:
    ...
    return {'title': title, 'company': company, 'location': 'Japan', 'type': job_type, 'salary': salary, 'description': description[:5000], 'skills': skills, 'apply_url': apply_url, 'posted_date': _clean(entry.get('pubDate')), 'is_remote': True, 'country': 'Japan'}
...
def fetch_remoteok_japan_entries(session: requests.Session, log) -> List[Dict]:
    try:
