...
class WeWorkRemotelyScraper(BaseScraper):
    ...
    def scrape(self, keywords: List[str], locations: List[str], max_pages: int) -> List[Dict]:
        ...
        for feed_url in FEEDS:
            resp = safe_get(self.session, feed_url, headers={'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/124.0.0.0 Safari/537.36', 'Accept': 'application/rss+xml, application/xml'}, domain_key='weworkremotely')
            if resp is None:
