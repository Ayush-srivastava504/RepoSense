...
class AshbyScraper(BaseScraper):
    ...
    def _fetch_board(self, session, board: str) -> List[Dict]:
        ...
        for entry in entries:
            ...
            job = build_job(title=entry.get('title', ''), company=company_name, location=location, description=entry.get('descriptionPlain', '') or entry.get('description', ''), apply_url=entry.get('jobUrl', '') or entry.get('applyUrl', ''), is_remote=bool(entry.get('isRemote', False)) or None, posted_date=str(entry.get('publishedAt', '') or ''), source=self.source_name)
            if job:
                out.append(job)
