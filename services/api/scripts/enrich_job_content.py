...
# writes it back to the `jobs` table (see migration 017).
#
# NOT SCHEDULED any more: content-enrichment.yml (which ran this) was removed,
# and job-content-enrichment.yml runs enrich_all_content.py --target jobs
# instead. Kept for its daily-trickle pattern (thin + never-enriched rows only,
# small --limit 100 default). Beware: unless --no-fallback is passed it stores
# template-fallback text whenever Groq is unavailable.
#
# Note: blog posts are authored manually as JSON under content/blog/ and are
# NOT auto-generated by anything in this repo — the old daily blog-post
# generator script has been removed.
